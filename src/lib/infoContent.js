  ...
  privacy: {
    en: {
      ...
      sections: [
        ...
        { h: "Cookies & local storage", p: "We use first-party cookies for your sign-in session and the bd_aid analytics identifier, and browser local storage for your theme preference, language, and in-progress order drafts. We never use advertising cookies. Clearing your browser's cookies and site data removes them." },
    ...
    ko: {
      ...
      sections: [
        ...
        { h: "쿠키·로컬 저장소", p: "로그인 세션과 bd_aid 분석 식별자는 자사 쿠키에, 테마 설정·언어·작성 중인 주문 드래프트는 브라우저 로컬 저장소에 저장합니다. 광고 쿠키는 사용하지 않습니다. 브라우저의 쿠키와 사이트 데이터를 지우면 함께 삭제됩니다." },
    ...
    zh: {
      ...
      sections: [
        ...
        { h: "Cookie 与本地存储", p: "我们使用第一方 Cookie 保存登录会话与 bd_aid 分析标识，使用浏览器本地存储保存主题偏好、语言与进行中的订单草稿。我们从不使用广告 Cookie。清除浏览器的 Cookie 与站点数据即会删除它们。" },
    ...
    es: {
      ...
      sections: [
        ...
        { h: "Cookies y almacenamiento local", p: "Usamos cookies propias para tu sesión de acceso y el identificador analítico bd_aid, y el almacenamiento local del navegador para tu preferencia de tema, idioma y borradores de pedido en curso. Nunca usamos cookies publicitarias. Al borrar las cookies y los datos del sitio del navegador se eliminan." },
