...
这个构建会固定使用 `VITE_DEMO_MODE=false` 和空的 `VITE_VENDOR_API_URL`（即 same-origin），不需要为 Vendor 前端新增生产 API 地址变量。主站与 Vendor Page 同域时只需已有的 `PUBLIC_ORIGIN=https://belovediamond.com`；邀请链接默认指向 `/vendor/`。只有 Vendor Page 改为独立域名或不同部署路径时，才需要用 `VENDOR_ORIGIN` 和 `VENDOR_APP_URL` 覆盖默认值。
...
5. **测试真实文件上传**：另外配置 COS 凭证、COS 源站地址和准确的 CORS Origin；桶可以保持私有，API 会为读取生成短期签名 URL。这不是注册、登录或密码重置的前提。
