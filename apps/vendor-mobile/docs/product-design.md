...
- 数据库：现有 PostgreSQL；迁移 `0016_supplier_portal.sql`、`0017_supplier_password_reset.sql`。
...
迁移新增 `suppliers`、`supplier_invites`、`supplier_password_reset_tokens`、`supplier_order_assignments`、`supplier_updates`、`supplier_inventory`，并把 `supplier` 加入现有通用 session 类型。每一条订单分配、更新和库存都以 `supplier_id` 隔离。密码重置 token 在数据库中只保存 SHA-256 hash、使用后立即失效；其他已登录设备保持登录。
