...
describe("어드민 실주문 콘솔", () => {
  ...
  it("이벤트 + 아티팩트 + 액션 → 고객 포털에 그대로 흐른다", async () => {
    ...
    expect(after.body.order.waitingOn).toBe("BELOVEDIAMOND");
  ...
  it("새 액션을 열면 이전 열린 액션은 취소된다 (한 번에 컨펌 하나)", async () => {
    ...
    expect(byCode[first.body.actionCode]).toBe("CANCELLED");
    expect(adm2.body.actions.find((a) => a.kind === "FINAL_QC_CONFIRMATION").status).toBe("OPEN");
  });
